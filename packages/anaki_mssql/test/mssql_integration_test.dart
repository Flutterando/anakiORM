// Integration test for anaki_mssql.
//
// PREREQUISITES:
// 1. Start SQL Server (e.g. docker compose up from example/shelf_mssql_example)
// 2. Build the Rust native library:
//    ./scripts/build_native.sh mssql --local
//
// Run with:
//    dart test test/mssql_integration_test.dart
...
void main() {
  ...
  group('MSSQL-specific types', () {
    test('BIT is returned as bool', () async {
      final rows = await db.query<Map<String, dynamic>>(
        'SELECT * FROM users WHERE name = @name',
        {'name': 'Ana'},
      );
