...
/// PostgreSQL driver for AnakiORM.
///
...
/// await db.open();
/// ```
class PostgresDriver implements AnakiDriver {
  final String _host;
  final int _port;
  ...
  late final AnakiPostgresBindings _bindings;
  bool _loaded = false;

  @override
  SqlDialect get dialect => SqlDialect.generic;

  /// Creates a new PostgreSQL driver.
  PostgresDriver({
  ...
  void _ensureLoaded() {
    ...
    try {
      _bindings = AnakiPostgresBindings.fromLibrary(_loadLibrary());
    } catch (_) {
      _bindings = AnakiPostgresBindings.fromNativeAssets();
    }
    _loaded = true;
  }
  ...
  DynamicLibrary _loadLibrary() {
    ...
    try {
      final libUri = Uri.parse('package:anaki_postgres/anaki_postgres.dart');
    ...
    if (Platform.isMacOS) {
      libName = 'libanaki_postgres.dylib';
      final arch = _isArm64() ? 'arm64' : 'x64';
      platformLibName = 'libanaki_postgres-darwin-$arch.dylib';
    } else if (Platform.isLinux) {
      libName = 'libanaki_postgres.so';
      final arch = _isArm64() ? 'arm64' : 'x64';
      platformLibName = 'libanaki_postgres-linux-$arch.so';
    } else if (Platform.isWindows) {
      libName = 'anaki_postgres.dll';
      platformLibName = 'anaki_postgres-windows-x64.dll';
    } else {
      throw const ConnectionException(
        'Unsupported platform',
        details: 'AnakiORM PostgreSQL supports macOS, Linux, and Windows.',
      );
    }
    ...
    final searchPaths = [
      ...
      'native_libs/$libName',
      'packages/anaki_postgres/native_libs/$platformLibName',
      'packages/anaki_postgres/native_libs/$libName',
