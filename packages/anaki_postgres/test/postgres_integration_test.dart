// Integration test for anaki_postgres.
//
// PREREQUISITES:
// 1. Start PostgreSQL (e.g. docker compose up from example/shelf_postgres_example)
// 2. Build the Rust native library:
//    ./scripts/build_native.sh postgres --local
//
// Run with:
//    dart test test/postgres_integration_test.dart
...
void main() {
  ...
  group('PostgreSQL-specific types', () {
    test('boolean type is returned correctly', () async {
      final rows = await db.query<Map<String, dynamic>>(
        'SELECT * FROM users WHERE name = @name',
        {'name': 'Ana'},
      );
