...
/// await db.open();
/// ```
class MysqlDriver implements AnakiDriver {
  final String _host;
  final int _port;
  ...
  void _ensureLoaded() {
    if (!_loaded) {
      ...
      _bindings = AnakiMysqlBindings.fromLibrary(lib);
      _loaded = true;
    }
  }
  ...
  DynamicLibrary _loadLibrary() {
    ...
    try {
      final libUri = Uri.parse('package:anaki_mysql/anaki_mysql.dart');
    ...
    if (Platform.isMacOS) {
      libName = 'libanaki_mysql.dylib';
      final arch = _isArm64() ? 'arm64' : 'x64';
      platformLibName = 'libanaki_mysql-darwin-$arch.dylib';
    } else if (Platform.isLinux) {
      libName = 'libanaki_mysql.so';
      final arch = _isArm64() ? 'arm64' : 'x64';
      platformLibName = 'libanaki_mysql-linux-$arch.so';
    } else if (Platform.isWindows) {
      libName = 'anaki_mysql.dll';
      platformLibName = 'anaki_mysql-windows-x64.dll';
    } else {
      throw const ConnectionException(
        'Unsupported platform',
        details: 'AnakiORM MySQL supports macOS, Linux, and Windows.',
      );
    }
    ...
    final searchPaths = [
      ...
      'native_libs/$libName',
      'packages/anaki_mysql/native_libs/$platformLibName',
      'packages/anaki_mysql/native_libs/$libName',
