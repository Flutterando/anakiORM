...
fn prepare_sql(
    ...
    crate::sql_params::rewrite(sql, params, crate::sql_params::Dialect::Postgres)
}
...
mod prepare_sql_tests {
    ...
    fn params_are_numbered_and_text_preserved() {
        ...
        let (out, values) = prepare_sql(
            "select 'coração' as c, @nome as n where idade > @idade",
            &params,
        );
        assert_eq!(out, "select 'coração' as c, $1 as n where idade > $2");
